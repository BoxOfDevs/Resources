...
if(count(glob("crashdumps/CrashDump*.log")) === 0){
	$phar = new Phar("BoxCore.phar");
	$phar->buildFromDirectory("plugins/BoxCore-master");
	echo "The BoxCore plugin is currently working well.\nTo download the plugin, go to:\nhttp://46.4.105.238:8080/job/BoxCore/lastSuccessfulBuild/artifact/BoxCore.phar\n\n";
	exit(0);
}else{
	echo "The BoxCore plugin has a syntax error.\nIt will be fixed whenever the developers have a chance.\nPlease be patient and wait for them to fix it.\nAlternatively, you can get a previous stable version here:\nhttp://46.4.105.238:8080/job/BoxCore/lastStableBuild/\n\n";
	exit(1);
}