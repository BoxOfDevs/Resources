...
if(count(glob("crashdumps/CrashDump*.log")) === 0){
	$phar = new Phar("CommandShop.phar");
	$phar->buildFromDirectory("plugins/CommandShop");
	echo "The CommandShop plugin is currently working well.\nTo download the plugin, go to:\nhttp://46.4.105.238:8080/job/CommandShop/lastSuccessfulBuild/artifact/CommandShop.phar\n\n";
	exit(0);
}else{
	echo "The CommandShop plugin has a syntax error.\nIt will be fixed whenever the developers have a chance.\nPlease be patient and wait for them to fix it.\nAlternatively, you can get a previous stable version here:\nhttp://46.4.105.238:8080/job/CommandShop/lastStableBuild/\n\n";
	exit(1);
}
